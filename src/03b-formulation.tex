...
\begin{definition}[Cost Function per Broadcast Interval]
    ...
    where $\Inorm{\vec{x}}$ denotes the $L^1$-norm of the vector $\vec{x}$, $\beta$ is the weight of overflow penalty\comments{, and $I[\cdot]$ denotes the \emph{indicator function} which is equal to $1$ when the statement is true and $0$ otherwise}.
\end{definition}
