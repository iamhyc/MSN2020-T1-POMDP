...
%FIXME: 
\comments{
    However, due to the constraint of broadcast information sharing on network topology, each AP only has its own OSI, however the optimal dispatching actions depends on the GSI and global \brlatency~information according to the RHS of equation (\ref{eqn:sp_0}).
    Thus, problem P1 is actually a POMDP, whose general solution is of huge complexity \cite{IJCAI03-NairR,IJCAI99-BoutilierC}.
    % \deny{as all the historical system states should be traced back in the current system state}
}
