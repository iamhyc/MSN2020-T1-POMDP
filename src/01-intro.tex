...
\comments{
    An motivating example. (about AoI, about partially observable)
    Without loss of generanality, we assume the network topology among APs are generated following \emph{Bar\'abasi-Albert model}, and each AP could only access to the neighbor edge server within one hop (i.e., the routing for multi-hops are unacceptable for information propagation).
}
%----------------------------------------------------------------------------------------%
