...
To alleviate the curse of dimensionality, we first use the baseline policy with fixed dispatching actions to approximate value function at the RHS of the Bellman's equations in equation (\ref{eqn:val_f}).
Specifically, the baseline policy is elaborated below.

\begin{policy}[Baseline Policy]
    In the baseline policy $\Baseline$, each AP fixes the target processing server for each job type as the previous broadcast interval. Specifically, at the $t$-th broadcast interval,
    {\small
    \begin{align}
...
\begin{proof}
    ...
    And the proof of $W_{\tilde{\Policy}}(\Stat) \leq W_{\Baseline}(\Stat)$ is similar to the proof of \emph{Policy Improvement Property} in Chapter II of \cite{dp-control}.
\end{proof}
