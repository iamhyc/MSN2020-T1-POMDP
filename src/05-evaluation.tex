...
\comments{
    The network topology among APs is generated according to Barab\'asi-Albert (BA) model \cite{albert1999diameter} and randomly place the processing servers collocated with the APs.
    After the execution of Algorithm \ref{alg_0}, the update period $N$ is $6$ reduced from $15$ before the subset partition.
    ...
    The arrival traces on APs and job processing time on processing servers are extracted,  and randomly chosen from \cite{clusterdata:Reiss2011}.
}
