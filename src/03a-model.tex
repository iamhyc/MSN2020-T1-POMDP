...
\begin{figure*}[htp!]
    ...
    \caption{\comments{(not altered yet)} The Illustration of System Model}
    \label{fig:system}
\end{figure*}
\comments{
    ...
    Specifically, the zero-th server denotes the cloud server, and the remaining elements of $\esSet$ denote the edge servers.
    The placement of edge servers are assumed co-located with APs
    % which would suffer from heavy communication overhead with other AP nodes but with powerful computation capacity.
}%

%FIXME: [colocation] dispatchers are implemented on APs.
...
\comments{
    It is assumed the the AP could only access to its own edge server, and the neighbor edge server without further routing (to express the partial observable information).
}
...
%----------------------------------------------------------------------------------------%
%FIXME: Rewrite this section, for integration
