...
\comments{
    Furthermore, it is assumed that one AP could only access to some neighbor edge servers due to transmission and signaling latency limitation.
}%
...
We refer to $\esSet_{k} \subseteq \esSet$ as the \emph{candidate server set} of the $k$-th AP, and $\apSet_{m} \subseteq \apSet$ as the \emph{potential AP set} of the $m$-th edge server ($\forall k\in\apSet, m\in\esSet$).
